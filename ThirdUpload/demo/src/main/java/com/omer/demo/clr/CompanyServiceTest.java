		...
		public void run(String... args) throws NotAllowedException, LoginException {
			...
			// getting all company's coupons before testing	
					...
					try {
						...
						PrintStringUtils.printedException(e.getMessage(), "trying to update SALE companyID");
					}
					c10.setCompanyID(4);
					...
					TablesAndLinesUtils.printCouponsTable(companyService.getCompanyCoupons(), "testing updateCoupon method (sale description updated)");
	
					TablesAndLinesUtils.printCouponsTable(companyService.getCompanyCoupons(Category.Sport), "testing getAllCouponsByCategory method (sport)");
				
					TablesAndLinesUtils.printCompaniesTable(Arrays.asList(companyService.getCompanyDetails()), "testing getCompanyDetails method (getting Abidas details)");

					TablesAndLinesUtils.printCouponsTable(companyService.getCompanyCoupons(), "testing deleteCoupon method (all Lululemon coupons before deleting)");
					
					companyService.deleteCoupon(c10);
					
					
					TablesAndLinesUtils.printCouponsTable(companyService.getCompanyCoupons(), "testing deleteCoupon method (all Lululemon coupons after deleting SALE)");
